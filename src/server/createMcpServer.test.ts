...
describe("createMcpServer", () => {
  ...
  test("refuse une écriture sans exécuter quoi que ce soit", async () => {
    ...
    const r = await invoquer(
      [outil],
      contexte({ allowWrites: false }),
      "ecrire",
      { cible: "x" },
    );
  ...
  test("rend le code métier d’une erreur GraphQL", async () => {
    const outil = defineTool({
      ...
      inputSchema: {},
      execute: async () => {
        throw new GraphQLCallError("GET_USER_USECASE_USER_NOT_FOUND", {
          messages: ["…"],
        });
      },
    });
  ...
  test("marque en clair les écritures visant la production", async () => {
    ...
    try {
      createMcpServer({
        ...
        ctx: contexte({
          target: {
            url: "https://api.siguri.happykiller.net/graphql",
            host: "api.siguri.happykiller.net",
            isProd: true,
          },
        }),
      });
...
/**
 * Ce que ces cas protègent est ce qu'un exploitant LIT au-dessus de données
 * réelles. En transport http, la cible était déclarée `isProd: false` faute de
 * pouvoir trancher — et la sortie annonçait « is_prod: false » au-dessus de
 * seize comptes de production. Constaté en recette.
 */
describe("is_prod dans le contrat de réponse", () => {
  const cible = (isProd?: boolean) =>
    contexte({
      target: {
        url: "http://api.interne:3000/graphql",
        host: "api.interne",
        isProd,
      },
      allowWrites: true,
  ...
  test("publie le drapeau quand la cible sait", async () => {
    assert.equal(
      corps(await invoquer([lecture], cible(true), "lire", {})).is_prod,
      true,
    );
    assert.equal(
      corps(await invoquer([lecture], cible(false), "lire", {})).is_prod,
      false,
    );
  });

  // Le cœur du correctif : ne rien dire plutôt que dire « non ».
  test("omet le drapeau quand la cible ne sait pas", async () => {
    const c = corps(await invoquer([lecture], cible(undefined), "lire", {}));
    assert.ok(!("is_prod" in c), "is_prod ne doit pas être publié");
    assert.equal(c.target, "http://api.interne:3000/graphql");
  });

  /** La description enregistrée, seule façon de voir le préfixe. */
  const descriptionDe = (ctx: ToolContext): string => {
    const { outil } = fabriqueEcriture();
    let vue = "";
    const proto: any = Object.getPrototypeOf(
      createMcpServer({ name: "t", version: "0", catalog: [], ctx }),
    ...
    try {
      createMcpServer({ name: "t", version: "0", catalog: [outil], ctx });
    } finally {
      proto.registerTool = original;
    }
    return vue;
  ...
  test("ne préfixe [PRODUCTION] que sur une certitude", () => {
    assert.match(descriptionDe(cible(true)), /PRODUCTION/);
